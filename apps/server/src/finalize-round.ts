...
export async function finalizeRound(db:PrismaClient,room:RuntimeRoom):Promise<RoundResults>{
 ...
 const players=room.players.map(player=>{const cells=room.settings.categoryIds.map(categoryId=>{const v=verdicts.get(`${player.id}:${categoryId}`)!;return{categoryId,text:player.answers[categoryId]??'',status:v.status,reasonCode:v.reasonCode,reasonFa:v.reasonFa,points:points.get(`${player.id}:${categoryId}`)??0}});const roundScore=cells.reduce((sum,cell)=>sum+cell.points,0);player.score=(player.score??0)+roundScore;return{playerId:player.id,name:player.name,roundScore,totalScore:player.score,cells}});
